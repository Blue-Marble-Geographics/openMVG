...
bool getInitialRotation(const std::string& filename, Mat3& rotation)
{
  bool ok = false;
#if BMG_EXTENSIONS
  std::unique_ptr<Exif_IO> exifReader(new Exif_IO_EasyExif);
  ...
  {
    double c1, c2, c3;
    if (exifReader->DJICameraPitch(&c1) && exifReader->DJICameraRoll(&c2) && exifReader->DJICameraYaw(&c3))
    {
      rotation = Mat3::Identity(); // TODO: compute
      ok = true;
    }
  }
#endif
...
int main(int argc, char **argv)
{
    ...
    ++iter_image, ++my_progress_bar )
  {
    ...
    if (getGPS(sImageFilename, i_GPS_XYZ_method, pose_center) && b_Use_pose_prior)
    {
      ...
      Mat3 prior_rotation;
      if (getInitialRotation(sImageFilename, prior_rotation))
      {
        v.b_use_pose_rotation_ = true;
        v.pose_rotation_ = prior_rotation;
      }
