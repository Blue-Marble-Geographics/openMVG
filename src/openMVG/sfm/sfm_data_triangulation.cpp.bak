...
)
{
  ...
  if (obs.size() >= 2)
  {
    FixedArray<Vec3, 8> bearing(cnt);
    FixedArray<Mat34, 8> poses(cnt);
    FixedArray<Pose3, 8> poses_(cnt);
    ...
    else
    {
      ...
      (
        ...
        bearing[0],
        poses_[cnt-1].rotation(),
        poses_[cnt-1].translation(),
        bearing[cnt-1],
        X,
        etri_method
      );
...
const
{
  ...
  for (IndexT i = 0; i < nbIter; ++i)
  {
    ...
    FixedArray<IndexT, 32> inlier_set(obs.size()); // Never larger
    size_t inlier_cnt = 0;
    ...
    for (const auto & obs_it : obs)
    {
      ...
      const auto tmp = cam.oneBearing(obs_it.second.x);
      if (!CheiralityTest(tmp, pose, X))
        continue;
  ...
  if (best_inlier_set_size >= min_required_inliers_)
  {
    ...
    for (size_t i = 0; i != best_inlier_set_size; ++i)
    {
      ...
      landmark.obs.emplace(val,obs.at(val));
    }
  }
